 ...
 const FILTERS = {
    ...
    async function toggleVideoRecording() {
        if (!isRecording) {
            ...
            recordingCanvas.width = videoElement.videoWidth || 1280;
            recordingCanvas.height = videoElement.videoHeight || 720;
    ...
    class Particle {
        constructor() {
            ...
            // Added more random initial velocity for energy
        ...
        draw() {
            ...
            if (this.active) {
                if (this.isTextPixel) {
                    // Convert hex color to HSL for the text particles
        ...
        drawToContext(tempCtx) {
            // Simplified draw method for photo capture
            const rgb = hexToRgb(currentTextColor);
            const hsl = rgbToHsl(rgb.r, rgb.g, rgb.b);
            const h = hsl.h;
            const s = hsl.s * 100;
    ...
    function scanText(text) {
        ...
        if (text !== "") {
            // Use the current text color for rendering
    ...
    function animate() {
        ...
        // --- Video Recording Compositing Logic ---
        if (isRecording) {
            // Clear recording buffer
            recordingCtx.clearRect(0, 0, recordingCanvas.width, recordingCanvas.height);
            
            recordingCtx.filter = getComputedStyle(videoElement).filter;
